...
class ApiLogController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (! in_array(Auth::user()->role, ['technician', 'admin'], true)) {
            return response()->json(['error' => 'Only technicians and administrators can create logs.'], 403);
        }
    ...
    public function update(Request $request, Log $log)
    {
        ...
        if (! in_array($user->role, ['technician', 'admin'], true)) {
            return response()->json(['error' => 'Only technicians and administrators can update logs.'], 403);
        }
