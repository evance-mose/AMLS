...
class ApiIssueController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (! in_array(Auth::user()->role, ['custodian', 'admin'], true)) {
            return response()->json(['error' => 'Only custodians and administrators can create issues.'], 403);
        }
    ...
    public function update(Request $request, Issue $issue)
    {
        ...
        if (! in_array($user->role, ['technician', 'admin'], true)) {
            return response()->json(['error' => 'Only technicians and administrators can update issues.'], 403);
        }
